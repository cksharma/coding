...
public class MainDecisionTree {
    ...
    private static void printTree(TreeNode rootNode) {
        ...
        while (queue.isEmpty() == false) {
            ...
            System.out.println();
            System.out.println("parent = " + Utils.valueReverseMap.get(parent.featureId).get(parent.value));
            ...
            for (TreeNode child : parent.childrenList) {
                sb.append(Utils.valueReverseMap.get(child.featureId).get(child.value)).append(" ");
                queue.add(child);
            }
            sb.append("]");
